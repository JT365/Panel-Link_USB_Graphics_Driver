/**
 * status-link_protocol.h
 *
 * Status-Link Protocol Implement
 *  - Weidong Zhou
 *
 * This is a Status-Link protocol implementation for USB Graphics project.
 *
 * Features
 *  - Implements Status-Link package per protocol spec.
 *  - No dynamic allocations.
 *
 * This library is coded in the spirit of the stb libraries and mostly follows
 * the stb guidelines.
 *
 * It is written in C99. And depends on the C standard library.
 * Works with C++11
 *
 *
 */

#ifndef _STATUS_LINK_PROTOCOL_H
#define _STATUS_LINK_PROTOCOL_H

#include <windows.h>

#define MIN_Buffer_Size 512

#pragma pack(push) //±£´æ¶ÔÆë×´Ì¬
#pragma pack(1)   // 1 bytes¶ÔÆë

typedef struct _STATUSLINK_INFO {
	unsigned short firmware_version;
	unsigned char panellink_version;
	unsigned char statuslink_version;
	unsigned char hardware_platform;
	unsigned char os_version;
	unsigned char sn[64];
	unsigned short screen_resolution_x;
	unsigned short screen_resolution_y;
	unsigned int storage_size;
	unsigned char max_brightness;
	unsigned char current_brightness;
}  STATUSLINK_INFO;

#pragma pack(pop)//»Ö¸´¶ÔÆë×´Ì¬

#ifdef __cplusplus
extern "C"
{
#endif

// - packageResetSL -
//   len 
//       in -  length of buffer
//       out - length of SL payload
//   value
//       in - value of backlight level.
//
int packageResetSL(unsigned char * data, unsigned int * len);

// - packageSetBLSL -
//   len 
//       in -  length of buffer
//       out - length of SL payload
//   value
//       in - value of backlight level.
//
int packageSetBLSL(unsigned char * data, unsigned int * len, unsigned char value);

// - packageSetTimeSL -
//   len 
//       in -  length of buffer
//       out - length of SL payload
//   value
//       in - value of backlight level.
//
int packageSetTimeSL(unsigned char * data, unsigned int * len, SYSTEMTIME * value);

// - packageGetTimeSL -
//   len 
//       in -  length of buffer
//       out - length of SL payload
//   value
//       in - value of backlight level.
//
int packageGetTimeSL(unsigned char * data, unsigned int * len);

// - depackGetTimeSL -
//   len 
//       in -  length of buffer
//       out - length of SL payload
//   time
//       in - value of system time.
//
int depackGetTimeSL(unsigned char * data, unsigned int len, SYSTEMTIME * time);

// - packageGetInfoSL -
//   len 
//       in -  length of buffer
//       out - length of SL payload
//   value
//       in - value of backlight level.
//
int packageGetInfoSL(unsigned char * data, unsigned int * len);

// - depackGetInfoSL -
//   len 
//       in -  length of buffer
//       out - length of SL payload
//   value
//       in - value of backlight level.
//
int depackGetInfoSL(unsigned char * data, unsigned int len, STATUSLINK_INFO * info);


int packagePushSL(unsigned char * data, unsigned int * len);
int packageResetPLSL(unsigned char * data, unsigned int * len);
int packageResetANSL(unsigned char * data, unsigned int * len);

#ifdef __cplusplus
}  // extern C
#endif

#endif // _STATUS_LINK_PROTOCOL_H
